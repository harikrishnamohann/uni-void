...
Initializing an infinite list (capacity = 0) of integers:
  List numbers = list_init(0, sizeof(int));
...
Example:
  int data = *(int*)list_remove_from(&my_list, front);
...
// use capacity = 0 if list needs to scale infinitly.
// unit_size is capacity of a single element that has to be
// inserted in the list.
// example usage: if you want an infinite list of integers,
// initialize list as: 
// List integers = list_init(0, sizeof(int));
// or a list of max 20 employees:
// List employees = list_init(20, sizeof(struct Employee));
